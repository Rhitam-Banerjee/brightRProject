...
const BestSellersPopular = () => {
  ...
  const getBooks = async () => {
    ...
    setAllBooks(responses[responses.length - 1]);
  };
  useEffect(() => {
    getBooks();
  }, [age]);
  return (
    <section>
      <h1 className="mt-[100px] pl-2 w-full max-w-7xl m-auto font-semibold text-[0.8rem] xs:text-[0.6rem]">
        Browse Books for {age}-{age + 1} years old
      </h1>
      <h1 className="mt-[10px] pl-2 w-full max-w-7xl m-auto font-bold text-[2rem] xs:text-[1.2rem]">
          ...
          if (name !== "" && image?.length > 10 && isValidImageUrl) {
            ...
            return (
              <SwiperSlide
                ...
                key={index}
                className="startCarouselSlideMulti h-auto !w-[200px] group bg-transparent"
              >
                <div
...
export default BestSellersPopular;
