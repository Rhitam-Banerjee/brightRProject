...
const GlobalBestSeller = () => {
  const imageUrlRegex = /\.(jpeg|jpg|gif|png)$/i;
  const { age } = useSelector((store) => store.age);
  const [allBooks, setAllBooks] = useState([]);
  const getBooks = async () => {
    const responses = await axios
      .get(`http://localhost:5000/globalBestSeller?age=${age}`)
      .then((res) => {
        return res.data;
      })
      .catch((err) => {
  ...
  return (
    <section>
      <h1 className="mt-[100px] pl-2 w-full max-w-7xl m-auto font-bold text-[2rem] xs:text-[1.2rem]">
        Global Best Sellers
      </h1>
      <Swiper
        slidesPerView={"auto"}
...
export default GlobalBestSeller;
